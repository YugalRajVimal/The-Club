...
export interface ISignupSession extends MongooseDocument {
  ...
  consent: {
    ...
    consentVersion: string;
    signedName: string;
    // Base64 PNG data URL captured from the frontend's canvas signature
    // pad. Stored as a plain string alongside the other consent fields
    // rather than routed through storageService — a drawn signature is
    // small (typically 5-20KB as a data URL) and always read/written
    // together with the rest of the consent record, so a separate
    // upload/URL round-trip isn't worth the complexity here.
