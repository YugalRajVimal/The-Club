...
export const signupConsent = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!signupSessionId || consentAccepted !== true || !consentVersion || !signedName || !signatureImage) {
    throw Errors.validation(
      "signupSessionId, consentAccepted (true), consentVersion, signedName, and signatureImage are required"
    );
  }
...
export const userLogin = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Fallback for a missed/misconfigured Cashfree webhook: if this user has
  // any payment stuck in "created", re-check it with Cashfree now and
  // update membershipStatus/Payment/Receipt accordingly before responding.
  // Never blocks or fails login — see reconcilePendingPayments's own
  // try/catch in paymentController.ts.
  await reconcilePendingPayments(user._id.toString());
 
  // Re-fetch: reconcilePendingPayments may have just changed membershipStatus.
  const freshUser = (await User.findById(user._id)) ?? user;
 
  const token = signUserToken({
    userId: freshUser._id.toString(),
    membershipStatus: freshUser.membershipStatus,
  });
  return sendSuccess(res, { user: freshUser, token });
});
