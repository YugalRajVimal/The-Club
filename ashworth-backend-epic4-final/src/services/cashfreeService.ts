...
export interface CreateOrderResult {
  // The MERCHANT order_id you sent — this is what GET /pg/orders/{order_id}
  // and the webhook payload's data.order.order_id both key off. Cashfree's
  // own internal numeric cf_order_id is a different identifier entirely and
  // is NOT accepted by the order-status-fetch path — using it there 404s
  // with "order_not_found" (this bit us: the previous version of this
  // function preferred cf_order_id here, which silently broke every
  // reconciliation/verify call after order creation).
  orderId: string;
  // Cashfree's own internal numeric order id, kept only for logging/audit —
  // never used to look anything up.
  cfInternalOrderId: string | null;
  paymentSessionId: string;
  raw: unknown;
}
...
export async function createCashfreeOrder(params: CreateOrderParams): Promise<CreateOrderResult> {
  ...
  return {
    ...
    cfInternalOrderId: response.data.cf_order_id ?? null,
    paymentSessionId: response.data.payment_session_id,
    raw: response.data,
  };
}
...
// Authoritative status check — this is what /verify calls server-side. Never
// trust a client-supplied "success" flag; always re-check with Cashfree.
// `orderId` here MUST be the merchant order_id (Payment.cfOrderId in our
// schema — poorly named in hindsight, but it's the merchant id, not
// Cashfree's internal cf_order_id — see CreateOrderResult above).
export async function fetchCashfreeOrderStatus(orderId: string): Promise<OrderStatusResult> {
  const response = await axios.get(`${CASHFREE_BASE_URL}/orders/${orderId}`, {
    headers: cashfreeHeaders(),
  });
