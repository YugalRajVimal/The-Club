...
//   return (
//     <main className="bg-beige min-h-[80vh] flex items-center justify-center">
//       <Script
...
//       <div className="max-w-lg bg-white mx-auto  p-8">
...
function PaymentFlow() {
  ...
  // Creates the order, then immediately opens checkout once it succeeds.
  const loadOrderAndCheckout = useCallback(async () => {
    setPhase('creating_order');
    setErrorMessage(null);
    try {
      const res = await createPaymentOrder();
  ...
  // Once an order exists (created by the button click) and the SDK is
  // ready, open checkout automatically. This effect never runs on mount —
  // `order` starts out null and is only ever set inside
  // loadOrderAndCheckout, which itself only runs from the button's
  // onClick handler.
  useEffect(() => {
    if (phase === 'ready' && order && sdkReady && !hasStartedCheckout.current) {
      startCheckout();
    }
  }, [phase, order, sdkReady, startCheckout]);
